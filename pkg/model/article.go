...
// Article 文章
type Article struct {
	ID      int            `gorm:"column:id;primaryKey;size:32" bson:"id"`                // ID, store自行控制
	Author  string         `gorm:"column:author;not null;size:128" bson:"author"`         // 作者名
	Slug    string         `gorm:"column:slug;not null;uniqueIndex;size:128" bson:"slug"` // 文章缩略名
	Title   string         `gorm:"column:title;not null;size:128" bson:"title"`           // 标题
	Count   int            `gorm:"column:count;not null;size:32" bson:"count"`            // 评论数量
	Content string         `gorm:"column:content;not null" bson:"content"`                // markdown内容
	SerieID int            `gorm:"column:serie_id;not null;size:32" bson:"serie_id"`      // 专题ID
	Tags    pq.StringArray `gorm:"column:tags;type:json" bson:"tags"`                     // tags
	...
	DeletedAt time.Time `gorm:"type:datetime;column:deleted_at;not null" bson:"deleted_at"`                  // 删除时间
	UpdatedAt time.Time `gorm:"type:datetime;column:updated_at;default:current_timestamp" bson:"updated_at"` // 更新时间
